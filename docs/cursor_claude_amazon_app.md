...
28. [収益化戦略 — 5 人の視点で設計する](#28-収益化戦略--5人の視点で設計する)
29. [収益化の実装 — アフィリエイト・KDP・フリーミアム](#29-収益化の実装--アフィリエイトkdpフリーミアム)
30. [週末開発を複利にする — 48 時間実験と資産化](#30-週末開発を複利にする--48時間実験と資産化)
...
| **Bezos 型** | 何を約束するか | 「最近何読んだ？に 3 秒で答えられる」を顧客の変化から逆算する |
...
> — 「最近何読んだ？」に answering できなかった会社員が、1 年分の読了リストを 3 秒で見せられるようになった —
>
> yonda は Audible・Kindle・公共図書館・紙の本の記録を毎朝自動で同期する。ユーザーが入力する項目は原則ゼロ。読了した本は AI が書評ポイントを生成し、次に読む本まで提案する。

このプレスリリースが約束しているのは機能ではなく変化です（「答えられなかった人が、答えられるようになる」）。書いてみて心が動かないなら、作る前に企画を直します。**コードを 1 行も書かずに没にできるのが、この手法の最大の価値です。**
...
伝える順番も再設計します: ① 痛みの承認（「読んだ本、思い出せない」）→ ② 約束（1 文）→ ③ 根拠は 1 つだけ（自動同期）→ ④ 最初の行動（Google ログイン）→ ⑤ 証拠（ユーザーの読了数）→ ⑥ 価格（無料で始まることだけ先に言う）。
