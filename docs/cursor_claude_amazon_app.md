...
参考アプリ「yonda」は筆者が Cursor + Claude を使って週末だけで開発した読書記録 Web アプリです。完成までの設計判断・詰まったポイント・AI との対話方法をそのまま記録しています。執筆時点の最新モデルは Anthropic の **Claude Fable 5**（Claude 5 ファミリー）で、本書のコード生成・リファクタ・ドキュメント生成の手法はそのまま適用できます。

あなたが作るアプリは yonda と同じである必要はありません。読書記録に限らず、「複数のデータソースを統合して AI で付加価値をつける」Web アプリ全般に応用できる構造です。そして後半の収益化編は、**個人開発アプリ全般に使える「週末×AI×資産化」の戦略** としてまとめています。

### このガイドで学べること

- Cursor + Claude（Fable 5 世代）を使った **週末開発の進め方**
...
- テスト・監視・コスト管理の実践
- **収益化編**: アフィリエイト・KDP 出版・フリーミアムを組み合わせ、週末開発を複利にする戦略と実装
...
技術編（1〜27章）でアプリを作り、収益化編（28〜30章）でそれをお金に変える仕組みを設計します。すでにアプリを持っている方は収益化編から読み始めても構いません。

---

## 本書を読むためのガイド

全 30 章を 15 のテーマにまとめました。詳細な章立ては巻頭の目次を参照してください。はじめてアプリを作る方は 1 から順に、経験者は必要なテーマだけ拾い読みできます。収益化だけ知りたい方は 14〜15 から読み始めても構いません。

1. **全体像をつかむ**（1章）— システム構成とデータの流れ。何をどの順で作るかをここで把握します
2. **開発環境と Flask の土台**（2〜3章）— プロジェクト作成から API 設計・キャッシュまで、アプリの骨格を作ります
3. **Google ログインとマルチユーザー**（4〜5章）— 認証と「ユーザーごとのデータ分離」。公開アプリにする分岐点です
4. **Audible・Kindle 連携**（6〜7章）— 本書の核。非公式 API と OTP 認証を突破して読書記録を自動で取ります
5. **図書館と紙の本**（8〜9章）— どの市区町村図書館にも対応できる汎用アダプタ設計と、カメラ + AI での紙の本登録
6. **書誌情報の補完とデータベース**（10〜11章）— Google Books / Open Library / AI の 3 段構えと Firestore 統合
7. **ユーザーごとの自動同期**（12章）— Cloud Scheduler で毎朝全ユーザーの記録が勝手に貯まる仕組み
8. **AI 書評と AI 選書**（13〜14章）— 読了本の書評ポイント生成と、読書履歴からの推薦。AI で付加価値をつけます
9. **フロントエンド UI**（15章）— フレームワークなしの Vanilla JS で作る理由と実装パターン
10. **アフィリエイトとコミュニティ**（16〜17章）— Amazon 連携リンクと「みんなの読書」。収益化編の伏線です
11. **デプロイと開発の実務**（18〜20章）— Cloud Run 本番公開、ローカルデバッグ、テスト戦略
12. **性能・運用・セキュリティ・コスト**（21〜24章）— 本番で困る前に読む章。月数百円で運用する方法まで
13. **トラブルと AI ツールの選び方**（25〜26章）— 実際にハマった問題集と、AI エディタ・モデルの比較
14. **Cursor + Claude 開発術**（27章）— AI への頼み方・分割の仕方・プロンプト集。本書の方法論の核心
15. **収益化編**（28〜30章）— 5 人の視点で戦略を設計し、アフィリエイト・KDP・サブスクを実装し、週末開発を複利にする
...
技術編でアプリは動くようになりました。ここからは「動くアプリ」を「収益を生む資産」に変える戦略を設計します。
...
|---|---|---|---|
| ① Amazon アソシエイト | 数時間（16 章で実装済み） | 従量・成果報酬 | 月数百円〜数万円 |
...
16 章で実装したアフィリエイトタグは、単なる収益化ではなく**ユーザー機能と一体化**させるのが要点です。yonda では「気になる本リスト」から Amazon・Audible・ブックオフ・図書館（カーリル）への横断検索リンクを生成しており、アフィリエイトタグはその URL に自然に付与されます。ユーザーにとっては「一番お得な入手方法を探す」機能、運営にとっては収益源です。
...
収益の前にコストを固定します。yonda の月額コスト（24 章の実測ベース）:

| 項目 | 月額 | 備考 |
|---|---|---|
| Cloud Run | ほぼ 0 円 | 無料枠内（リクエスト数百万/月まで） |
| GCS + Firestore | 数十円 | データ量に依存 |
| AI API（書評・選書） | 数十〜数百円 | Haiku/Flash クラスを使い分け（13 章） |
...
| 土曜 4〜12h | 最小実装（Cursor + Claude で。27 章の進め方） | 機能フラグ付きで実装、本番デプロイ |
...
- デプロイ手順を 2 回打った → `deploy.sh` に固める（18 章）
- 同じ形の連携先を 2 つ実装した → 汎用アダプタに抽象化する（8 章）
- 同じプロンプトを 2 回書いた → プロンプト集・skill としてリポジトリに置く（27 章）
...
| **P0'** | 最初の 1 ヶ月 | 技術編 1〜18 章でアプリを本番デプロイ。アフィリエイトタグを組み込む | 技術編 × 29-1 |
