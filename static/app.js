...
const GENRE_NORMALIZE_MAP = {
  // 小説: 家族・恋愛（サブジャンル依存で normalizeGenre が動的に分岐）
  '小説: 家族・恋愛':            '小説: 家族・恋愛',
  // 小説: 現代文学（旧: 文学・フィクション）
  '文学・フィクション':          '小説: 現代文学',
  '小説: 現代文学':              '小説: 現代文学',
  '小説':                        '小説: 現代文学',
  // ミステリー・サスペンス（旧: ミステリー・スリラー・サスペンス）
  'ミステリー・スリラー・サスペンス': 'ミステリー・サスペンス',
  'ミステリー・スリラー':             'ミステリー・サスペンス',
  'ミステリー・サスペンス':           'ミステリー・サスペンス',
  'ミステリー':                       'ミステリー・サスペンス',
  // 自伝・回顧録系 → エッセイ（エッセイ sub あり）or 自己啓発（それ以外）
  // ※ エッセイ sub がある場合は normalizeGenre の essayMains チェックで先にキャッチされる
  '自伝・ノンフィクション':          '自己啓発',
  '自伝・回顧録':                     '自己啓発',
  'ノンフィクション':                 '自己啓発',
  // ビジネス・キャリア
...
function normalizeGenre(genreStr) {
  ...
  // ── 小説: 家族・恋愛 / 小説: 現代文学 分岐（文学/小説/フィクション系メイン）──
  const novelMains = ['文学', '小説', 'フィクション', 'fiction', 'literature'];
  if (novelMains.some(kw => mainLow.includes(kw))) {
    const romanceKws = ['恋愛', '家族', '青春', '親子', '夫婦', 'ロマンス', '恋人', '子育て', '愛情'];
    if (subParts.some(s => romanceKws.some(kw => s.toLowerCase().includes(kw)))) return '小説: 家族・恋愛';
    return '小説: 現代文学';
  }
  ...
  if (['人間関係', '子育て', 'コミュニケーション', 'parenting', 'relationship'].some(kw => genreLow.includes(kw)))
    return '人間関係・教育';
  if (['歴史', 'history', '文化史', '宗教史', '文明史'].some(kw => genreLow.includes(kw)))
    return '歴史・文化';
...
const CATEGORY_TAG_BLOCKLIST = {
  '小説: 家族・恋愛':            new Set(['文学', '小説', 'フィクション', 'fiction', '文学・フィクション', '文芸', '恋愛', 'ロマンス', '家族', '青春']),
  '小説: 現代文学':              new Set(['文学', '小説', 'フィクション', 'fiction', '文学・フィクション', '文芸', 'ドラマ', 'drama']),
  'ミステリー・サスペンス':      new Set(['ミステリー', 'スリラー', 'サスペンス', 'ホラー', 'フィクション', 'fiction', '小説', 'エンタメ']),
  'エッセイ':                    new Set(['エッセイ', '随筆', '体験記', 'essay', '文学', 'フィクション', 'ノンフィクション', '自伝', '回顧録']),
  ...
  '政治・社会':                  new Set(['政治', '社会', '政治学', '社会科学', '政治学・社会科学']),
  '歴史・文化':                  new Set(['歴史', 'history', '歴史学', '文化', '宗教', '戦争', '軍事']),
...
const DETAIL_TAG_PATTERNS = {
  '小説: 家族・恋愛': [
    ...
    { tag: '感動・泣ける',       words: ['感動', '涙', '泣ける', '切ない', '号泣', '心打つ'] },
    { tag: '死・喪失・悲哀',     words: ['死', '喪失', '悲しみ', '亡くなる', '別れ', '悲劇', '追悼'] },
    { tag: '海外・翻訳文学',     words: ['海外', '外国', '翻訳', '英米', 'ヨーロッパ', 'アメリカ', 'イギリス', 'フランス'] },
  ],
  '小説: 現代文学': [
    { tag: '職場・仕事',         words: ['仕事', '職場', '会社', 'サラリーマン', '転職', '出世', '同僚', '上司'] },
    { tag: '成長・自己発見',     words: ['成長', '自己発見', '変化', '旅立ち', '挑戦', '夢', '再生'] },
    { tag: '戦争・時代背景',     words: ['戦争', '戦時', '兵士', '戦場', '空襲', '特攻', '江戸', '明治', '昭和', '侍', '武士', '戦国'] },
    { tag: 'SF・ファンタジー',   words: ['sf', '宇宙', '未来', 'ロボット', 'タイムトラベル', '異世界', '魔法', 'ファンタジー', 'fantasy'] },
    { tag: '社会・人間ドラマ',   words: ['格差', '差別', '孤独', '社会', '人間', '葛藤', '生きる'] },
    { tag: 'コメディ・ユーモア', words: ['笑い', 'コメディ', 'ユーモア', 'おかしい', '笑える', 'comedy', 'humor'] },
    { tag: '海外・翻訳文学',     words: ['海外', '外国', '翻訳', '英米', 'ヨーロッパ', 'アメリカ', 'イギリス', 'フランス'] },
  ],
  'ミステリー・サスペンス': [
    { tag: '謎解き・推理',       words: ['謎', '推理', '殺人', '事件', '探偵', '刑事', '犯罪', '犯人', '容疑者', 'トリック', 'mystery', 'detective'] },
    { tag: 'ホラー・恐怖',       words: ['ホラー', '恐怖', '怖い', '幽霊', '呪い', '恐ろしい', 'horror'] },
    { tag: 'サスペンス・逃走',   words: ['サスペンス', 'スリラー', '心理戦', '追跡', '逃亡', 'thriller', 'suspense'] },
    { tag: 'SF・未来犯罪',       words: ['sf', '未来', 'ロボット', 'サイバー', '仮想現実', 'ディストピア'] },
    { tag: '歴史ミステリー',     words: ['江戸', '明治', '昭和', '侍', '武士', '戦国', '幕末', '時代小説'] },
    { tag: '法廷・法律',         words: ['法廷', '裁判', '弁護士', '検察', '判決', '冤罪', '裁判員'] },
    { tag: '国際・スパイ',       words: ['スパイ', '国際', '諜報', 'テロ', '外交', '陰謀', '工作員'] },
    { tag: '社会派・格差',       words: ['格差', '差別', '貧困', '社会問題', '不平等', 'いじめ'] },
  ],
  'ビジネス・キャリア': [
    { tag: 'エッセイ・随筆',       words: ['エッセイ', '随筆', '日記', '雑感', '考察', 'コラム', 'essay'] },
    { tag: '有名人・著名人',       words: ['有名人', '著名人', '芸能人', '政治家', '経営者', '起業家', '作家', 'celebrity'] },
    { tag: 'ジャーナリズム・調査', words: ['ルポ', 'ルポルタージュ', '調査', '取材', 'ジャーナリズム', 'ドキュメント'] },
    ...
    { tag: 'コミュニケーション',   words: ['コミュニケーション', '対話', '交渉', '説得', '共感', 'communication'] },
    { tag: 'キャリア・転職',       words: ['キャリア', '転職', '就職', '仕事術', '働き方', 'ワークライフ', 'career'] },
    { tag: 'イノベーション・DX',   words: ['イノベーション', 'dx', 'デジタル変革', '変革', 'トランスフォーメーション', 'innovation'] },
    { tag: '会計・財務・投資',     words: ['会計', '財務', '決算', '損益', 'ファイナンス', '投資', '株', '資産', 'accounting'] },
    { tag: '税金・確定申告',       words: ['税金', '確定申告', '税務', '国税', '控除', '節税', '源泉徴収', '納税', '所得税', '経費'] },
  ...
  '自己啓発': [
    ...
    { tag: '挑戦・逆境',           words: ['挑戦', '逆境', '失敗', '壁', '乗り越え', '転機', '再起', '起業家', 'ceo', '経営者'] },
    { tag: 'リーダー・人物',       words: ['リーダー', '偉人', '人物', '伝記', '社長', '政治家', '科学者', '芸術家'] },
  ],
  '人間関係・教育': [
  ...
  '政治・社会': [
    ...
    { tag: '中国・アジア',           words: ['中国', '習近平', '共産党', '台湾', 'アジア', '北朝鮮', '韓国', '拘束', 'スパイ'] },
    { tag: '哲学・思想',             words: ['哲学', '思想', '倫理', '道徳', '存在', '自由', '幸福', '正義', 'philosophy'] },
    { tag: 'ジェンダー・フェミニズム', words: ['ジェンダー', 'フェミニズム', 'フェミニスト', '女性', '男女', '性差', '賃金格差', '育児', '家事', 'feminist', 'gender', '母娘', '家父長'] },
  ...
  '歴史・文化': [
    ...
    { tag: '歴史的人物',             words: ['偉人', '英雄', '武将', '将軍', '王', '皇帝', '革命家', '歴史', '伝説'] },
  ...
  'ライフ': [
    ...
    { tag: 'フィットネス・スポーツ', words: ['運動', 'フィットネス', 'スポーツ', 'トレーニング', '筋肉', 'ランニング', 'マラソン', 'exercise'] },
    { tag: '旅行・アウトドア',       words: ['旅行', '旅', 'アウトドア', '登山', '観光', '散歩', '街道', 'travel', 'ガイド', 'ひとり旅'] },
    { tag: '日常・暮らし',           words: ['暮らし', '生活', '日常', '家事', '台所', '掃除', '整理', '片付け', '丁寧な暮らし', 'ライフスタイル'] },
    { tag: '読書・本',               words: ['読書', '本', '書籍', '図書館', '名著', '文学', 'ノート', '読み方', '文章', '語彙', 'バズる'] },
    { tag: 'アート・デザイン',       words: ['アート', '美術', 'デザイン', 'クリエイティブ', '絵', '写真', '芸術', 'art'] },
    { tag: '音楽・映画・エンタメ',   words: ['音楽', '映画', 'エンタメ', '映像', '演劇', 'ライブ', 'アニメ', 'music', '映画館'] },
  ...
  '科学・テクノロジー': [
    ...
    { tag: 'データサイエンス',         words: ['データ', '統計', '分析', 'データサイエンス', 'ビッグデータ', 'analytics'] },
    { tag: 'セキュリティ',             words: ['セキュリティ', 'ハッキング', 'サイバー', 'プライバシー', '暗号', 'security'] },
    { tag: '医療・バイオ',             words: ['医療', '医学', '遺伝子', 'バイオ', '病気', 'がん', 'ゲノム', '薬', 'medicine', 'biology', '生命科学'] },
    { tag: '脳科学・認知科学',         words: ['脳', '脳科学', '神経', '認知', '意識', '心理学', '行動', '感情', 'ニューロン', '認知科学', '数覚'] },
    { tag: '人類学・考古学',           words: ['人類学', '考古', '化石', 'ネアンデルタール', '旧石器', '縄文', '人類起源', '人類進化', '霊長類', '先史'] },
    { tag: '宇宙・天文',               words: ['宇宙', '天文', 'ブラックホール', '天体', '星', '惑星', '銀河', 'space', '宇宙論'] },
    { tag: '物理・数学',               words: ['物理', '量子', '相対性', '素粒子', '数学', '論理', '確率', 'physics', 'math', '三体問題'] },
    { tag: 'ロボット・自動化',         words: ['ロボット', '自動化', 'オートメーション', '自律', 'robot'] },
    { tag: 'デジタル・インターネット', words: ['インターネット', 'デジタル', 'web', 'sns', 'ソーシャル', 'digital'] },
    { tag: '工学・エンジニアリング',   words: ['工学', 'エンジニアリング', '土木', '建築', '機械', '電気', 'engineering'] },
  ],
};
...
/** カテゴリ分布バーを描画 */
const CAT_COLORS = {
  '小説: 家族・恋愛':            { read: '#e699c8', unread: '#f5cce8' },
  '小説: 現代文学':              { read: '#6d9eeb', unread: '#bdd3f5' },
  ...
  '政治・社会':                { read: '#b07ab8', unread: '#ddb8e4' },
