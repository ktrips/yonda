...
// カテゴリ名と同義のタグ（クラウド表示から除外）
const CATEGORY_SELF_TAGS = {
  '大衆小説':       new Set(['大衆小説', '小説', 'エンターテインメント']),
  '文学・その他小説': new Set(['文学', '小説', '純文学', '文芸', '文学・その他小説']),
  'ノンフィクション': new Set(['ノンフィクション', 'エッセイ', '随筆']),
  'ビジネス':       new Set(['ビジネス', 'ビジネス書']),
  '自己啓発':       new Set(['自己啓発']),
  '政治・経済':     new Set(['政治', '経済', '政治・経済']),
  '社会・ライフ':   new Set(['ライフ', 'ライフスタイル', '社会']),
  'テクノロジー・科学': new Set(['テクノロジー', '科学', 'it']),
};

// ジャンル文字列のキーワードでカテゴリを決定
// ─ 定義順が優先順位（上ほど先に判定）─
const TAG_CATEGORY_GENRE_KEYWORDS = {
  // ── ノンフィクションを先に確認（エッセイ等が小説に誤分類されないよう） ──
  'ノンフィクション': [
    'ノンフィクション', 'エッセイ', '随筆', '伝記', '自伝', '回顧録',
    'ルポ', 'ドキュメンタリー', '評伝', '体験記',
    '芸能', 'タレント', '有名人', '証言', '実録',
    'memoir', 'essay', 'biography', 'nonfiction', 'true crime',
  ],
  // ── 大衆小説（ジャンル系フィクション） ──
  '大衆小説': [
    'ミステリー・サスペンス', 'sf・ファンタジー', 'ミステリー・サスペンス・ホラー',
    'ミステリー', '推理', 'サスペンス', 'スリラー', 'ホラー',
    'sf', 'ファンタジー', '異世界', 'ロマンス', '恋愛小説',
    '時代小説', '大衆小説', 'ライトノベル', 'コミック', '漫画', '絵本', '児童書',
  ],
  // ── 文芸・純文学 ──
  '文学・その他小説': [
    '純文学', '文芸', '文学・小説', '日本文学', '外国文学', '海外文学',
    '現代文学', '歴史小説', '児童文学', '詩', '戯曲',
    '小説', 'フィクション', '文学', 'fiction', 'drama',
  ],
  // ── テクノロジー（ビジネスより先に確認） ──
  'テクノロジー・科学': [
    'コンピュータ・テクノロジー', 'サイエンス・テクノロジー', 'コンピュータ・it',
    'テクノロジー', 'ai', '人工知能', 'プログラミング', 'コンピュータ',
    'it', 'サイエンス', '科学', '医学', '生物', '物理', '数学',
    'エンジニアリング', 'データ', 'デジタル', '宇宙', '工学', '医療',
    'technology', 'science',
  ],
  // ── ビジネス（経営・経営戦略・マーケ系） ──
  'ビジネス': [
    'ビジネス・キャリア',
    'ビジネス', '経営', 'マネジメント', 'マーケティング', '起業',
    'リーダーシップ', 'キャリア', '仕事術', 'イノベーション',
    'business', 'management', 'marketing',
  ],
  // ── 自己啓発（心理・習慣・人間関係系） ──
  '自己啓発': [
    '自己啓発・マインドフルネス',
    '自己啓発', 'スピリチュアル', '心理', '脳科学', '認知', '教養',
    'コミュニケーション', '人間関係', 'マインドフルネス', '瞑想', '習慣',
  ...
  '政治・経済': [
    '社会・政治', '人文・思想', '資産・金融',
    '政治', '経済', '金融', '投資', '資産',
    '外交', '国際', '歴史', '哲学', '倫理', '思想', '宗教', '社会学',
    '民主主義', '選挙', 'politics', 'economics', 'philosophy', 'history',
  ],
  // ── 社会・ライフスタイル ──
  '社会・ライフ': [
    'ライフスタイル・健康', 'アート・エンターテインメント',
    'ライフ', 'ライフスタイル', '社会問題', '格差', '環境',
    '料理', '健康', 'スポーツ', '旅行', '趣味', '家族',
    '教育', '子育て', 'ファッション', 'アート', '音楽', '映画',
    '育児', '美容', 'ガーデニング', 'インテリア', '語学',
    'アウトドア', '食', 'グルメ', 'life', 'lifestyle',
  ],
};
...
const DETAIL_TAG_PATTERNS = {
  '大衆小説': [
    ...
    { tag: '家族・親子',           words: ['家族', '親子', '夫婦', '母親', '父親', '兄弟', '姉妹', '子ども', '子供', '息子', '娘', '両親'] },
    { tag: '青春・学生',           words: ['青春', '高校生', '中学生', '大学生', '学生', '受験', '部活', '文化祭', '進路', '学校', '少年', '少女'] },
    ...
    { tag: 'SF・未来',             words: ['sf', '宇宙', '未来', 'ロボット', 'タイムトラベル', 'クローン', 'サイバー', '仮想現実', 'ディストピア'] },
    ...
    { tag: '冒険・旅',             words: ['冒険', '旅', '探検', '旅人', 'adventure'] },
    { tag: 'コメディ・ユーモア',   words: ['笑い', 'コメディ', 'ユーモア', 'おかしい', '笑える', 'comedy', 'humor'] },
  ],
  '文学・その他小説': [
    { tag: '純文学・文芸',       words: ['純文学', '文芸', '芸術的', '難解', '実験的'] },
    { tag: '海外文学',           words: ['海外', '外国', '翻訳', '英米', 'ヨーロッパ', 'アメリカ文学', 'フランス文学'] },
    { tag: '恋愛・ロマンス',     words: ['恋愛', '恋人', 'ロマンス', '片思い', '初恋', '結婚', '告白', 'love'] },
    { tag: '家族・親子',         words: ['家族', '親子', '夫婦', '母親', '父親', '兄弟', '姉妹', '子ども', '子供'] },
    { tag: '青春・成長',         words: ['青春', '学生', '成長', '少年', '少女', '若者', '旅立ち'] },
    { tag: '歴史・時代',         words: ['歴史', '時代', '江戸', '明治', '昭和', '戦国', '幕末', '古代'] },
    { tag: '戦争・平和',         words: ['戦争', '平和', '兵士', '戦場', '空襲', '戦時'] },
    { tag: '感動・泣ける',       words: ['感動', '涙', '泣ける', '切ない', '号泣'] },
    { tag: '死・喪失・再生',     words: ['死', '喪失', '悲しみ', '亡くなる', '別れ', '再生'] },
    { tag: '社会・人間ドラマ',   words: ['格差', '差別', '孤独', '社会', '人間', '葛藤', '生きる'] },
    { tag: '児童・絵本',         words: ['子ども向け', '絵本', '児童', '小学生', '幼児'] },
  ],
  'ノンフィクション': [
    { tag: 'エッセイ・随筆',         words: ['エッセイ', '随筆', '日記', '雑感', '考察', 'essay'] },
    { tag: '伝記・自伝',             words: ['伝記', '自伝', '回顧録', '評伝', '生涯', '人物', 'memoir', 'biography'] },
    { tag: '有名人・芸能',           words: ['芸能', 'タレント', '有名人', 'アーティスト', '俳優', '著名人', '経営者', '政治家'] },
    { tag: 'ルポ・調査報道',         words: ['ルポ', 'ルポルタージュ', '調査', '取材', 'ジャーナリズム', 'ドキュメンタリー'] },
    { tag: 'ノンフィクション犯罪',   words: ['犯罪', '事件', '殺人', '冤罪', '詐欺', '組織犯罪', '事件の真相'] },
    { tag: '歴史ノンフィクション',   words: ['歴史', '史実', '記録', '出来事', '事件', '時代', 'history'] },
    { tag: '旅・体験記',             words: ['旅', '旅行', '体験', '冒険', '探検', 'travel'] },
    { tag: '社会・問題提起',         words: ['社会問題', '格差', '差別', '貧困', '環境問題', '提言'] },
    { tag: '科学・自然ノンフィクション', words: ['科学', '自然', '生物', '動物', '植物', 'nature'] },
    { tag: '戦争・歴史証言',         words: ['戦争', '外交', '国際', '証言', '戦後', 'war'] },
  ],
  'ビジネス': [
    ...
    { tag: '経営・企業',           words: ['経営', '企業', '事業', '会社経営', 'ceo', '経営者'] },
    { tag: '営業・セールス',       words: ['営業', 'セールス', '売上', '顧客獲得', '提案', 'sales'] },
    { tag: '生産性・仕事術',       words: ['生産性', '仕事術', '効率', 'タスク', 'プロジェクト管理', 'productivity'] },
  ],
  '自己啓発': [
    { tag: '習慣・ルーティン',     words: ['習慣', 'ルーティン', '朝活', '継続', '習慣化', 'habit'] },
    { tag: 'マインドフルネス・瞑想', words: ['マインドフルネス', '瞑想', '呼吸', '意識', '集中', 'mindfulness'] },
    { tag: '心理学・思考法',       words: ['心理', '行動', '認知', '意思決定', 'バイアス', '脳', 'ナッジ', 'psychology'] },
    { tag: 'コミュニケーション',   words: ['コミュニケーション', '人間関係', '対話', '交渉', '説得', '共感', 'communication'] },
    { tag: '目標・夢の実現',       words: ['目標', '夢', '成功', '達成', 'ビジョン', '実現', 'goal'] },
    { tag: '幸福・生きがい',       words: ['幸福', '幸せ', '生きがい', '充実', '豊かさ', '満足', 'happiness'] },
    { tag: 'メンタル強化',         words: ['メンタル', 'ストレス', '不安', 'うつ', '折れない', 'レジリエンス', 'mental'] },
    { tag: '自己理解・強み',       words: ['自己理解', '強み', '才能', '個性', '自分を知る', 'strength'] },
    { tag: '教養・知識',           words: ['教養', '知識', '学び', '思考力', '知的', '読書', 'education'] },
  ],
  '政治・経済': [
    ...
    { tag: '国際・外交',     words: ['外交', '国際', '外国', '世界', '条約', '安全保障', 'international'] },
    { tag: '歴史・近現代史', words: ['歴史', '近代', '現代史', '昭和史', '戦後', '幕末', '明治維新', 'history'] },
  ...
  '社会・ライフ': [
    ...
    { tag: '旅行・アウトドア',       words: ['旅行', '旅', 'アウトドア', '登山', '海外', '観光', 'travel'] },
    { tag: 'アート・デザイン',       words: ['アート', '美術', 'デザイン', 'クリエイティブ', '絵', '写真', '芸術', 'art'] },
    { tag: '音楽・映画',             words: ['音楽', '映画', 'エンタメ', '映像', '演劇', 'ライブ', 'アニメ', 'music'] },
    { tag: 'マインド・メンタル',     words: ['メンタル', '心', 'ストレス', '不安', 'うつ', '癒し', '充実', 'mental'] },
    { tag: '語学・学習',             words: ['語学', '英語', '言語', '外国語', '学習', '勉強', 'language'] },
    { tag: '住まい・インテリア',     words: ['インテリア', '住まい', '家', '部屋', '収納', 'diy', 'interior'] },
    { tag: '社会問題・人権',         words: ['格差', '差別', '貧困', '社会問題', '不平等', 'いじめ', '孤独', '孤立', 'ジェンダー'] },
    { tag: '環境・サステナブル',     words: ['環境', 'サステナブル', 'エコ', 'ゼロウェイスト', '気候変動', 'sustainable'] },
  ],
  'テクノロジー・科学': [
...
/** book をカテゴリに分類（genre → summaryフォールバック） */
function classifyBookToMainCategory(book) {
  ...
  // フォールバック: summaryキーワードで推定（ジャンル不明・その他を再分類）
  const summaryRaw = ((book.full_summary || '') + ' ' + (book.summary || '')).toLowerCase();
  if (summaryRaw) {
    if (['エッセイ', '随筆', '伝記', '自伝', '芸能', 'タレント', 'ルポ'].some(w => summaryRaw.includes(w))) return 'ノンフィクション';
    if (['小説', '物語', '主人公', '謎解き', '推理', '刑事'].some(w => summaryRaw.includes(w))) return '大衆小説';
    if (['起業', 'マーケティング', '経営', '組織', '事業'].some(w => summaryRaw.includes(w))) return 'ビジネス';
    if (['習慣', '目標', '幸福', 'マインドフルネス', '自己理解', '人間関係'].some(w => summaryRaw.includes(w))) return '自己啓発';
    if (['投資', '経済', '政治', '歴史', '哲学'].some(w => summaryRaw.includes(w))) return '政治・経済';
    if (['料理', '健康', '子育て', '旅行', 'スポーツ', 'アート'].some(w => summaryRaw.includes(w))) return '社会・ライフ';
    if (['ai', 'プログラミング', '科学', 'テクノロジー', '医療'].some(w => summaryRaw.includes(w))) return 'テクノロジー・科学';
  }
  return 'その他';
}
...
function generateDetailedTagsForBook(book, mainCat) {
  const selfTags = CATEGORY_SELF_TAGS[mainCat] || new Set();
  const tags = new Set();
  ...
  if (key && bookInsightsCache[key]?.points) {
    bookInsightsCache[key].points.forEach(p => {
      ...
      if (h && h.length <= 20 && !selfTags.has(h)) tags.add(h);
    });
  }
  // 2. ジャンルの2階層目以降
  genreTags(book.genre).slice(1).forEach(g => {
    if (g && g.length <= 20 && !selfTags.has(g)) tags.add(g);
  });
  ...
  for (const { tag, words } of patterns) {
    if (!selfTags.has(tag) && words.some(w => summaryText.includes(w))) tags.add(tag);
  }
  return [...tags];
}
...
function renderCategoryCloud(catData) {
  ...
  const catTotal = books.length || 1; // カテゴリ内冊数（パーセント計算用）
  ...
  const cloudHtml = sortedTagList.length
    ? sortedTagList.map(([tag, d]) => {
        ...
        const pct = Math.round((d.total / catTotal) * 100); // ← カテゴリ内占有率
        return `<span class="tag-chip" data-tag="${escapeAttr(tag)}" style="font-size:${size.toFixed(2)}rem;opacity:${opacity.toFixed(2)}" title="${escapeHtml(tag)}: このカテゴリの${pct}%（計${d.total}冊 / 読了${d.read} / 未読${d.unread}）">${escapeHtml(tag)}<span class="tag-chip-pct">${pct}%</span></span>`;
      }).join('')
