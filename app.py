...
#進め方
1. ユーザーのメッセージにMBTIタイプ（例：ENTJ、INTJ、INFP）や日本語の性格タイプ名（例：指揮官、建築家、仲介者）が含まれる場合、質問はせずにそのタイプに合う本を3冊、直ちに提案フォーマットで出力すること
2. そうでない場合、MBTIの4軸（E/I, S/N, T/F, J/P）について、短い質問を1つずつ出してユーザーの傾向を把握する
3. 質問はひとつずつ実行すること
4. 十分な情報が集まったら、推測したMBTIタイプを伝え、そのタイプに合う本を3冊提案する
...
MBTIタイプを提示する際は「ENTJということは〇〇な人で、」のようにそのタイプの特徴を1文で簡潔に説明してから本を提案すること。"""
...
AI_RECOMMEND_INIT_MESSAGES = {
    "5questions": "こんにちは。AI推しをお願いします。まず「5つの質問推しへようこそ！5つの質問で、あなたにピッタリな本を推すので、短くてもいいので答えて下さいね。」と挨拶した後、最初の質問（Q1. 今の気分）をしてください。",
